...
- **`/auth/callback`** handles post-OTP session setup. If the user has no `users` row and no `name` in Supabase user metadata (i.e., they came through `/login` rather than `/register`), redirects to `/onboard` before any DB writes. Otherwise: claims an open `org_invites` row atomically (UPDATE … RETURNING); falls back to domain/`allowed_emails` match if no invite. Creates the `users` record. Role: `admin` if the org has zero active users, `editor` otherwise. Status: `active` if the user was invited or is the first active user; `pending` otherwise (requires org admin approval). **Pending-user upgrade path:** if the existing user's status is `pending`, the callback checks for a newly-created invite before redirecting to `/pending` — if one is found, it's claimed atomically, the user is promoted to `active`, and the welcome email fires. Uses `NEXT_PUBLIC_APP_URL` for all redirects (not `request.nextUrl.origin`) to work correctly behind a reverse proxy.
...
- **`/register`** requires the email to match an approved org via domain, `allowed_emails`, or an open invite. `email_domain` is optional on registration — orgs without a unique shared domain can omit it and rely on `contact_emails`-seeded invites for self-registration. Returns a pending-org error if the email's domain or `contact_emails` matches an org still under review.
- **Org approval** (both `/api/admin/orgs/[id]/approve` and `/api/platform-admin/orgs/[id]/approve`) automatically seeds `org_invites` rows for all `contact_emails` on the org record. This ensures contact emails get `status = 'active'` (not `pending`) when they self-register after approval, and is the primary mechanism for no-domain orgs to register.
