...
export default async function StoryDetailPage({ params }: PageProps) {
  ...
  const { data: story } = await supabase
    ...
    .select(
      `
      *,
      organizations(id, name, website_url),
      story_assets(*)
    `
    )
    .eq('id', id)
  ...
  const org = story.organizations as { id: string; name: string; website_url: string } | null
      ...
      <div className="border-t border-wire-border pt-8">
        {isOwnOrg ? (
          <div className="bg-wire-bg border border-wire-border rounded p-4 text-sm text-wire-slate text-center">
            ...
            <Link href="/dashboard" className="text-wire-red hover:underline">
              dashboard
            </Link>
            .
          </div>
